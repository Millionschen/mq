...
public class MqConfig {
  ...
  private @Value("${mq.exchange.key}") String exchangeKey;

  //出错重试配置
  //重试次数
  private @Value("${mq.setting.retryNum}") int retryNum;
  //初次出错重试的间隔时间
  private @Value("${mq.setting.initRetryInterval}") int initRetryInterval;
  //出错重试间隔时间增加倍率
  private @Value("${mq.setting.retryMultiplier}") int retryMultiplier;
  //出错重试最大间隔
  private @Value("${mq.setting.retryMaxInterval}") int retryMaxInterval;
  ...
  @Bean MessageDispatcher messageDispatcher() {
